...
if echo "$ERROR" | grep -qi "too large\|too big\|size limit\|exceeds.*limit\|maximum.*size\|file is too long\|content too large"; then
    GUIDANCE="LARGE FILE RECOVERY: '${FILE_PATH}' is too large to read directly. Read it in parts with Read(offset/limit), narrow it with Bash(grep/sed -n), or delegate to a read-only search subagent that reads excerpts instead of whole files.
    Example: Agent(subagent_type=\"Explore\", prompt=\"In ${FILE_PATH}: [your question]\")"
