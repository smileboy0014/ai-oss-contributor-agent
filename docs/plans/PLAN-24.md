...
## 8. rev.4 — 구현 중 바뀐 것 (2026-09-26)

계획서를 「쓸 때 옳았던 문서」로 남기지 않는다. 구현이 계획을 고친 곳만 적는다.

### 8.1 🔴 테스트가 잡은 결함 둘 — 계획에 없던 수정

| 무엇 | 증상 | 왜 계획이 못 봤나 |
|---|---|---|
| `resolvePending` 의 **검증 순서** | 사유가 빈 요청이 거부되기 **전에** `aiContributionAllowed = true` 가 대입됐다. 예외는 났지만 엔티티는 `allowed=true · resolvedAt=null` — **기계 판정으로 위장한 허용** | 계획은 「무엇을 거부하는가」만 정했고 **거부가 어떤 상태를 남기는가**를 묻지 않았다. 롤백이 가려 준다고 가정한 셈이다 |
| `cancelSelection` 의 **출발 상태** | `ANALYZED` 인 후보에 `POST /reject` 가 통했다. 전이표에 `ANALYZED → REJECTED` 가 있어서다(`rejectAsInfeasible` 의 길) | 계획은 전이를 **표**로만 봤다. 표가 허용하는 것과 **이 메서드가 뜻하는 것**이 다를 수 있다는 것을 놓쳤다 |

둘 다 **UseCase 테스트가 DB 를 다시 읽어** 잡았다. 반환값만 단언했으면 둘 다 초록이다.

⚠️ 두 번째는 단순한 누락이 아니다. 고치지 않으면 `cancelSelection` 이
`rejectAsInfeasible` 과 **같은 것**이 되고, 둘이 같아진 순간 다음 사람이 메서드 하나로
합친다. 그 시점에 Q-5 의 「자동 취소 경로를 만들지 않는다」를 붙들고 있는 것이 없다.

### 8.2 계획과 달라진 설계

| 항목 | 계획(rev.3) | 실제 | 왜 |
|---|---|---|---|
| `reanalyze` 가드 | `resolvedAt` 이 있으면 재분석 거부 | **방향을 본다** — 허용(`TRUE`)으로 되돌리는 것만 거부, 금지(`FALSE`)로 조이는 것은 통과 | 양방향으로 얼리면 대상 저장소가 나중에 AI 금지를 추가해도 영영 못 본다. **되돌릴 수 없는 쪽**은 그쪽이다 |
| `assertContributionAllowed` | 그대로 둔다 | `clearanceFor` 에 **위임** | 판정 로직 두 벌은 한쪽에만 새 규칙이 들어간다. 「같은 클래스의 두 문이 같은 기준으로 판정한다」를 테스트로도 고정했다 |
| `resolve` 반환 | `void` | `Instant`(해소 시각) | 응답이 `resolvedAt` 을 실어야 한다. 엔티티를 돌려주면 LAZY 가 트랜잭션 밖으로 샌다 |
| 규율 ④ ArchUnit | 규칙 1개 | **2개로 분리** | 원래 규칙이 `AnalyzeIssuesUseCase → AnalyzeRepositoryPolicyUseCase` 를 위반으로 잡았다. 🔴 규율 ④는 남의 UseCase 호출을 **막는 게 아니라 그것만 허용**한다. 막을 것은 ⓐ 남의 **adapter** 의존 ⓑ 그 호출이 `application` **밖**에서 일어나는 것 |
| ArchUnit 임포트 | 미끼만 제외 | **테스트 클래스도 제외**(`DO_NOT_INCLUDE_TESTS`) | 빼지 않으면 규율 ④ 규칙이 **테스트**를 문다. 테스트는 픽스처를 세우려고 남의 애그리거트를 정당하게 조립한다 |
| 통행증 테스트 픽스처 | 언급 없음 | `PolicyClearanceFixtures` (`repository.domain` 의 `src/test`) | 생성자가 패키지 가시성이라 **거기서만** 만들 수 있다. 「값 픽스처를 애그리거트 너머로 공유하지 않는다」의 예외 — `PolicyClearance` 는 규율 ④의 값 타입 예외라 막을 의존이 애초에 없다 |
| `clearance()` 단위 테스트 | 도메인 테스트 | **UseCase 통합 테스트**로 | `repository.getId()` 를 쓰므로 영속되지 않은 엔티티로는 검증할 수 없다. 실제 조회 경로와 같은 트랜잭션 안이라 LAZY 초기화도 함께 걸린다 |

### 8.3 딸려 나온 것 — 거부 메시지가 우회법을 안내하고 있었다

```
"금지 판정은 해소로 뒤집을 수 없다 — 사람이 DB 를 고쳐야 한다 (S-5 · FR-2)"
```

게이트가 막으면서 **자기를 지나가는 법**을 적어 뒀다. 지웠고
`거부_메시지가_우회법을_안내하지_않는다_S5` 로 회귀를 걸었다.

같은 종류가 `#58`(시크릿 스캐너)에서 두 번 나왔다 — 안내문이 화이트리스트·인용 기호를
권했고 둘 다 우회 경로였다. 🔵 **「조치 방법은 게이트를 통과하는 법이어야 하고 우회하는
법이면 안 된다」**를 규약에 적을 자리는 `safety-boundaries.md` 요약 표 아래라고 보고,
**별도 PR 로 분리**했다 — 승인 게이트 PR 에 규약 개정을 섞으면 둘 다 흐려진다.
