...
#### S-5 ④ — 일시적 실패는 보류로 굳히기 전에 지연·재시도

`external-deps.md` — 「`X-RateLimit-Remaining` 이 임계 미만이면 **작업을 실패시키지 말고 지연**시킨다.
리밋 소진은 **정상 운영 상황**이다」. 그런데 보류는 사람만 푼다. 정상 운영 상황을
수동 개입 대상으로 굳히면 안 된다.

- 리밋·5xx·타임아웃은 **재시도 후에도 실패할 때만** 보류한다
- `pending_reason` 에 **사유 코드**(`RATE_LIMITED`·`SERVER_ERROR`·`TOO_LARGE`·`TRUNCATED`)를 남겨
  #24 가 일괄 재시도 대상을 고를 수 있게 한다
