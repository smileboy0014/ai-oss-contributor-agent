...
| **Filter** | 규칙 기반 1차 배제 — 요구사항 불명확 · 대규모 아키텍처 변경 · ~~종료됨~~. **대외 호출을 하지 않는다** (#9).<br>⚠️ 「종료됨」은 구현돼 있으나 **발화하지 않는다** — 수집이 `state=open` 고정이라 닫힌 이슈가 데이터에 없다 (#14).<br>「활성 PR 존재」는 여기가 아니라 **#11 입구 + #22** 다 — S-2 방어의 이전 |
