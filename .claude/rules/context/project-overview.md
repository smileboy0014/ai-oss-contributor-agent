...
✅ **이제 관측할 수단이 있다** (2026-09-26 · #25) — `/actuator/metrics` 에 LLM 토큰(입·출력) · 파이프라인 단계별 소요 시간 · 안전 게이트 통과/차단 · 후보 상태 분포가 나온다.
⚠️ 구현·테스트·리뷰·PR 지표는 **그 단계가 아직 없어** 재지 않는다. 그 단계를 만드는 이슈가 자기 메트릭을 함께 넣는다.
