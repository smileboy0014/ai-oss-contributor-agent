...
#### 🟡 판단 기록 — **아직 닫지 않는다** (2026-09-26 · #14)

#14 가 「스캔이 API 스레드를 점유한다」는 바로 그 문제에 부딪혔고, **비동기로 흡수하고
Q-3 은 열어 두기로** 했다. 위 판단 기준 둘이 아직 충족되지 않았기 때문이다.

| 기준 | 현재 |
|---|---|
| 동시 실행 2건 이상이 필요해질 때 | 전용 풀 `core=max=1` 로 **1건 고정**이다. 필요해지지 않았다 |
| API 응답 지연이 관측될 때 | #14 가 **202 + 비동기**로 없앴다 |

지금 가르면 **배포 대상이 없어 분리가 실제로 도는지 검증할 수단이 없고**, 아래 #42 숙제까지
함께 판단해야 해 범위가 폭발한다.

🔴 **열어 둔 대가를 정확히 적어 둔다 — 「`@Profile` 한 줄」이 아니다.**

| 경로 | 닫을 때 드는 비용 |
|---|---|
| 스케줄러 → 파이프라인 | `@Profile("worker")` **한 줄** (`ScanScheduler`·`SchedulingConfig`) |
| **API → 파이프라인** | 🔴 **큐가 필요하다.** 컨트롤러(web)가 `LaunchScanUseCase`(worker)를 **같은 JVM 에서 직접 호출**한다. 가르면 web 에 그 빈이 없어 깨진다 — PRD §21 의 Redis Streams 가 그 자리다 |
| 중복 방어 | 🔴 **DB 락 + 마이그레이션.** `ScanExecutionRegistry` 가 프로세스 메모리라 인스턴스가 둘이 되면 **중복 스캔 차단이 사라진다.** 주인은 #26(「다중 인스턴스 대비 잠금(ShedLock 등)」) |

즉 이 항목을 닫는 일은 **큐 도입 + 분산 락**이고, 셋 중 둘의 주인이 이미 #26 이다.
...
#### ✅ 해소 경로 — `POST /api/repositories/{id}/policy/resolution` (2026-09-26 · #24)

| 확정 | |
|---|---|
| 「해소」 ≠ 「허용」 | 사람이 읽고 **금지로 닫는 것도 정상적인 해소**다. 허용 전용으로 두면 금지 판정을 내리려고 DB 를 손으로 고치게 되고, 그쪽이 더 위험하다 |
| 정책 행이 **없으면** 404 | 만들어 주면 **「읽지 않고 허용」** 이 되어 S-5 가 정면으로 뚫린다. 분석을 먼저 돌린다. 「보류」(기록됨)와 「행 없음」(기록조차 없음)을 가른다 |
| 금지 판정은 해소로 뒤집지 않는다 | 409. FR-2 가 API 호출 한 번으로 풀린다 |
| 근거(`note`)는 필수 | 왜 그렇게 판단했는지가 없으면 그 판정은 재검토도 못 한다. 스크럽 후 저장된다 (S-4) |
| `pending_reason` 은 **보존** | `resolved_at` 이 이미 「보류 아님」을 말한다. 비우면 왜 보류였는지가 사라진다 |

#### ✅ 해소 뒤 재분석 — **방향으로 가른다** (2026-09-26 · #24)

사람이 푼 판정을 재분석이 조용히 덮으면 Q-8 확정 ②가 무의미해진다. 그렇다고 양방향으로
얼리면 **대상 저장소가 나중에 AI 금지를 추가해도 영영 못 본다.**

| 해소 후 | 재분석 결과 | |
|---|---|---|
| 허용(`TRUE`) | `TRUE` | ⛔ 거부 — 자동이 사람 판단을 다시 쓰지 않는다 |
| 허용(`TRUE`) | `FALSE` (금지 문구 발견) | ✅ **통과** — 즉시 금지로 조인다 |
| 금지(`FALSE`) | 무엇이든 | ⛔ 거부 (#7 이 이미 막는다) |

가르는 것은 보수성의 정도가 아니라 **실패의 방향이 되돌릴 수 있는가**다 —
[`external-deps.md`](./external-deps.md) 의 같은 기준이다.

🔵 **남은 구멍 하나** — 규약이 바뀌었는데 그 문서를 **계속 못 읽는** 경우.
보류 판독(`NULL`)으로 기존 판정을 덮지 않는 것은 #7 이 세운 규칙이라 해소와 무관하게
그 전부터 있었다. 닫으려면 **문서 해시·ETag 비교**가 필요하고 #24 범위를 넘는다 — **#68**.

⚠️ 「못 읽었다」와 「읽었는데 그대로다」가 **지금 구분되지 않는다.** 둘 다 판정이 안 바뀐다.
#24 의 비대칭 가드는 금지로 **조이는** 재분석을 통과시키므로 규약 변경 자체는 반영된다 —
막히는 것은 **못 읽을 때뿐**이다.
...
| 2026-09-26 | smileboy0014 | Q-5 부분 구현 · Q-8 해소 경로와 재분석 방향 확정 (#24) |
