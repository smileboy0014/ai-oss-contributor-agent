...
**여기에 딸려 오는 숙제 하나** (2026-09-25 · #42)

테스트 대역 프로필 `fakes` 는 `web`/`worker` 와 **직교**하는 축이라 선택지를 줄이지 않는다.
다만 **런타임 가드를 이 항목이 닫힐 때까지 미뤄 뒀다.**

`SPRING_PROFILES_ACTIVE=fakes` 로 앱을 띄우면 대외 어댑터가 통째로 빠진다
(`@ExternalAdapter` = `@Profile("!fakes")`). 「운영에서 켜지면 기동 실패」 가드를 넣고 싶었으나
**「운영인지」를 코드가 판정할 방법이 없다** — 배포 프로파일이 바로 이 항목에서 미정이기 때문이다.

지금 방어는 **충돌 불가능한 이름 + 문서**뿐이다(`ExternalAdapter` javadoc · [`setup.md`](../../docs/setup.md)).
⚠️ **이 항목을 닫을 때 런타임 가드를 함께 판단한다.**

### Q-4. 🟡 샌드박스 의존성 해석과 네트워크 — **Gradle 한정 부분 확정** (2026-09-26 · #17)

S-3 은 네트워크 차단을 요구하는데 Gradle/Maven 빌드는 의존성을 받아야 한다.

**사용자 결정** — 2단계 실행 + 읽기전용 의존성 캐시. 미러 프록시(인프라 필요)와
「네트워크를 그냥 연다」는 기각했다.
...
② 씨딩     cp -a <workspace>/.gradle/caches/modules-2 /dependency-cache/modules-2
           네트워크 X · 캐시 볼륨 RW · 워크스페이스 RO · 대상 코드를 돌리지 않는다
③ 실행     GRADLE_RO_DEP_CACHE=/dependency-cache  (볼륨 RO · 네트워크 none)
           GRADLE_USER_HOME=<workspace>/.gradle   (RW — wrapper·락이 여기 있다)
```

🔴 **워밍이 볼륨을 잡지 않는 것이 핵심이다.** `GRADLE_USER_HOME` 은 의존성 캐시만 있는
곳이 아니라, 신뢰할 수 없는 빌드 스크립트가 `init.d/*.gradle` 을 심을 수 있는 자리다.
그것이 볼륨에 남으면 **다음 워밍에서 네트워크가 열린 채 자동 실행**되고, 볼륨은 후보
수명을 넘겨 지속된다. 씨딩을 따로 두어 볼륨에 들어가는 것을 `modules-2` 하나로 좁혔다.

🔴 **워밍·씨딩·실행이 같은 워크스페이스를 써야 한다.** wrapper 배포본이 거기 쌓이고,
wrapper 부트스트랩은 Gradle 이 시작되기 **전**이라 `--offline` 이 닿지 않는다.
다른 워크스페이스를 주면 실행 첫 명령이 「distribution 다운로드 → network=none」으로 죽는다.

#### 🔴 아직 닫지 않는다 — 실측 미완

| 남은 것 | 왜 |
|---|---|
| toolchain 자동 프로비저닝 | 빌드가 요구하는 JDK 가 이미지와 다르면 네트워크를 탄다 |
| dynamic version · SNAPSHOT | RO 캐시로 해결되지 않는다. `spring-kafka` 가 Spring SNAPSHOT 을 참조할 수 있다 |
| **Maven** | 로컬 저장소를 읽기전용으로 못 쓴다(`_remote.repositories`). **지원하지 않는다고 실패시킨다** — 조용히 네트워크를 여는 것이 최악이다 |
| 다중 워커 동시성 | 저장소당 워밍 1건을 **프로세스 내 락**으로 막는다. 다중 인스턴스에서는 성립하지 않는다 — Q-3 과 함께 |

**종결 조건** — `spring-kafka` 로 워밍 → 씨딩 → 오프라인 실행을 **한 번 통과**시킨다.
워크스페이스를 채우는 주체(**#18**)가 있어야 가능하다. #18 이슈에 인계해 두었다.

⚠️ 「✅ 확정」으로 닫지 않는 이유 — Phase 2 의 Maven 대상(`spring-boot`)에서 이 항목을
다시 열어야 하는데, 닫아 두면 그때 근거가 사라진다.

#### 잔여 위험 — 밖으로 나가는 것만이 아니다

워밍은 **네트워크가 열린 채 임의 코드를 실행**한다. 호스트는 안전하다(바인드 2개 · 소켓
없음 · 시크릿 없음 · 자원 상한). 그러나 —

- **밖으로** — 트래픽을 막지 못한다. 미러 프록시가 있어야 하고 MVP 범위 밖이다
- **안으로** — 기본 `bridge` 면 컨테이너가 **호스트 게이트웨이**로 나가 우리 PostgreSQL·
  Redis 에 닿는다. 전용 네트워크로 줄이지만 **완전 차단은 아니다**

### Q-5. ✅ 「사람이 고른다」의 실제 UI — **별도 `select` 를 둔다** (2026-09-25 · #24)

#### 확정 ① — `POST /api/candidates/{id}/select` 를 신설한다

`implement` 가 겸하게 하지 않는다. **선택과 실행은 성격이 다른 행위**다.

| | 선택 | 실행 |
|---|---|---|
| 성격 | 트리아지 — 「이건 해볼 만하다」 | 착수 — LLM 호출 + 샌드박스 **최대 30분** |
| 비용 | 없음 | 토큰 + 시간 |
| 빈도 | 스캔 1회에 수십 건을 훑는다 | 골라낸 소수 |

「관심 있다」가 곧바로 30분짜리 실행이 되면 **고를 수가 없다.** 동시 실행 제어(Q-3)도
이 분리가 있어야 가능하다 — 고른 것과 돌리는 것이 같으면 큐가 성립하지 않는다.

**S-6 의 「선정 · 착수 · PR 생성」 셋과 1:1 로 맞는다**는 점이 결정적이다.
`SELECTED` 가 찰나의 상태가 되면 「사람이 골랐다」가 DB 에 남지 않아,
승인 게이트 통과를 사후에 증명할 수 없다.

#### 확정 ② — `SELECTED → REJECTED` 를 연다 (사람 행위로만)

지금 `SELECTED` 에서 나가는 전이는 `IMPLEMENTING` 뿐이라, **잘못 고르면 빠져나갈 길이 없다.**
실행 비용이 30분·LLM 과금이라 실수 비용이 크다.

`SELECTED` 는 종단이 아니므로 S-6 의 「종단에서 나가는 전이 금지」에 걸리지 않는다.
**취소도 사람 행위로만** 일어난다 — 자동 취소 경로를 만들면 자동화가 끼어들 틈이 생긴다.

`REJECTED` 는 종단이므로 다시 고르려면 재분석이 필요하다. 이것은 의도다 —
번복이 가벼우면 승인이 가벼워진다.

#### ⚠️ 딸려 나온 것 — PRD §24 가 S-6 와 충돌한다

PRD **§23** 은 `implement`·`verify`·`pull-request` 를 **따로** 두는데,
**§24** 시퀀스 다이어그램은 `implement` 호출 **하나가** Fork → 계획 → 코딩 → 테스트 → 리뷰 →
**Draft PR 생성까지** 수행하는 것으로 그려져 있다.

§24 대로 구현하면 **PR 생성이 사람의 별도 승인 없이 나간다.** S-6 정면 위반이고 S-2 에도 닿는다.
**§23 이 맞고 §24 다이어그램이 틀렸다**고 본다 — PRD 개정 대상이다(#30).
...
**✅ 남은 것도 닫혔다** (2026-09-25 · #12) — 불변식 ⑧의 판정 필드는
**`contribution_candidate.attempt`** 다. `AgentRun` 컬렉션을 세지 않는다(애그리거트가 다르고
append-only 로 무한정 자란다).

절대 상한은 **도메인 상수** `ContributionCandidate.MAX_ALLOWED_ATTEMPTS = 3` 이고,
넘겨받은 값이 그보다 크면 거부한다. **상한을 올리려면 도메인 코드를 고쳐야 하고 그것이
리뷰에 보인다** — 설정 한 줄로 상한을 사실상 없애는 경로를 막는 것이 요점이다.

⚠️ `maxAttempts < 1` 만 막는 것으로는 부족하다. `0` 은 무한이 아니라 **최강 제약**이고
...
🔴 **운영 코드에도 같은 함정이 있다** (2026-09-26 · #17). 같은 docker-java 를 런타임에
쓰므로 **운영에서 같은 증상**이 난다. 운영은 전역 시스템 프로퍼티에 기대지 않는다 —
다른 라이브러리와 충돌하고, 설정이 코드 밖에 있으면 왜 그 값인지가 사라진다.
`SandboxConfig` 가 `withApiVersion(sandbox.docker-api-version)` 으로 **명시**한다.
⚠️ 이 값은 **핀이지 고정이 아니다** — 데몬이 이보다 낮으면 내려야 한다.
...
### Q-11. 🔵 Q-1(직접 구현)을 다른 대외 의존에 일반화하지 않는다 (2026-09-25 · #10)

미결이라기보다 **반복될 논쟁을 못 박아 두는 항목**이다.

Q-1 은 GitHub 에서 라이브러리를 버리고 `RestClient` 직접 구현을 택했다. 그 결정이 강해서
「이 프로젝트는 대외 의존을 직접 짠다」로 읽히기 쉽다. **그렇게 읽으면 틀린다.**

Q-1 의 근거 3개를 LLM 에 대보면 하나도 서지 않는다.

| Q-1 의 근거 | LLM 에서 |
|---|---|
| ETag 조건부 요청 + `updated_at` 커서를 직접 제어해야 한다 | 조건부 요청이 없다 |
| 2차 레이트리밋이 403 으로 와서 직접 구분해야 한다 | 429 가 정상적으로 온다 |
| `hub4j` 가 1년 넘게 RC | `anthropic-java` 는 2.x GA (2.65.0, 2026-09-22) |

그래서 #10 은 **공식 SDK** 를 택했다. 결론이 갈린 것이 일관성 부족이 아니라,
**경계마다 제약이 달랐던 것**이다.

**판단 기준** — 라이브러리를 쓸지 직접 짤지는 「우리 규율(레이트리밋·커서·에러 분류)을
라이브러리가 방해하는가」로 가른다. 방해하지 않으면 SDK 를 쓰고, **능력 인터페이스를 domain 에
두어 갈아끼울 수 있게** 한다. 그 이음매가 있으면 선택을 되돌리는 비용이 어댑터 한 장이다.

다음에 Docker·Redis 어댑터에서 같은 논쟁이 나올 자리다.
...
| 2026-09-25 | smileboy0014 | Q-5 확정 — select 신설 · 선택 취소 개방 · PRD §24 결함 발견 |
| 2026-09-25 | smileboy0014 | Q-11 신설 — Q-1 을 일반화하지 않는다 (#10) |
