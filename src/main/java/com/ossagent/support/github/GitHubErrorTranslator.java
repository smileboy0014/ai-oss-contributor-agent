...
public class GitHubErrorTranslator {
     ...
     * 시크릿까지 따라 들어올 수 있고, 어차피 {@link GitHubApiException} 생성자가
     * {@code TokenRedactor} 를 통과시키지만 양을 줄이는 것이 먼저다.
     */
    private static String excerpt(String body) {
        ...
        if (trimmed.length() > BODY_EXCERPT_LIMIT) {
            trimmed = trimmed.substring(0, BODY_EXCERPT_LIMIT) + "…(잘림)";
        }
        return " body=" + trimmed;
    }
}
