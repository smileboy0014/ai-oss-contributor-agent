...
for f in $files; do
  ...
  # 그러면 스프링을 쓰지 않는 평범한 JUnit 테스트가 ProcessBuilder 로 호스트에서 빌드를
  # 돌리거나 docker.sock 을 마운트해도 아무 장치가 막지 않는다. Q-9 가 「자동 스위트는
  # 실제 컨테이너를 띄우지 않는다」로 확정됐으므로 그 결정을 선언으로만 두지 않는다.
  #
  # Testcontainers 는 Java API 를 쓰지 프로세스를 띄우지 않으므로 걸리지 않는다.
  # 의도된 예외는 // safety-ok: <사유> 로 처리한다.
  if [ "$is_test" -eq 1 ]; then
    exec_why="테스트가 호스트에서 대상 저장소 코드를 실행하면 개발자 머신이 장악됩니다. 자동 스위트는 실제 컨테이너를 띄우지 않습니다 — Q-9."
    exec_alt="CodeSandbox 페이크로 갈음하세요. 컨테이너 제어 자체를 검증해야 하면 docker 호출을 기록하는 대역을 쓰고, 실제 컨테이너는 띄우지 않습니다."
  else
