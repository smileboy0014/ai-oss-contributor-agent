...
| **재시도·타임아웃은 adapter/out 에 명시** | 기본값에 맡기면 무한 대기가 생긴다. **상한은 세 축이다** — 아래 |
...
**세 번째 축이 왜 필요했나** (#16) — 이슈 #16 의 「검증 실패 시 재생성」과 Q-6 의
「`ANALYZE`·`PLAN` 은 파이프라인 재시도 없음」이 정면으로 읽혔다. **가르는 것은 실패의 종류다** —
전송 실패도 코드 실패도 아닌 **의미 실패**(스키마는 맞는데 내용이 틀렸다)를 다루는 축이 없었다.
Q-6 이 말한 「PLAN 은 재시도 없음」은 **루프 카운터** 이야기다.

⚠️ 파이프라인 축에 합치면 안 된다 — PLAN 에서 2번 태우면 정작 고쳐야 할 `CODE` 루프에
1회밖에 안 남는다. Q-6 이 「후보 전체 통합」을 기각한 이유가 그것이다.

가르지 않으면 **HTTP 5xx 한 번이 파이프라인 카운터를 태운다.** 후보가 코드 문제 없이
`FAILED` 로 떨어지고, 사람은 「AI 가 못 고쳤다」로 읽는다.

⚠️ 예산은 **곱해진다.** 구현 루프는 3 × (1 + 2) = 논리적 1회 시도당 대외 호출 최대 **9회**다
(첫 시도 + 재시도 2). 전송 상한을 올릴 때는 이 곱을 먼저 계산한다.

⚠️ **계획 축도 같은 곱을 탄다** — 2 × (1 + 2) = LLM 호출 최대 **6회**. 그 앞에 #15 의
GitHub 호출(최대 26 논리 호출 × 3)이 붙으므로, **후보 1건의 대외 호출은 세 구간의 합**이다.
한 축만 보고 「9회」라고 세지 않는다.
